...
function fit=benchmark_functions(L,Benchmark_Function_ID,dim,X,Y)
...
if Benchmark_Function_ID==24
    Bc = L(1);
    Ks=L(2);
    MdlGau = fitrsvm(X,Y,'BoxConstraint',Bc, 'KernelScale',Ks,'KFold',10,'KernelFunction','gaussian');
%      MdlGau = fitrsvm(training_set,y_training_set,'KFold',5,'KernelFunction','gaussian');
     fit = kfoldLoss(MdlGau);
     fprintf('The fitness is %8.5f\n',fit);
end
